...
class BinarySearchTree {
 ...
 public:
  ...
  protected:
    ...
    //helper fcn to move to next inorder node
    Node<KeyType, ValueType>* getNext(Node<KeyType, ValueType>* curr_)
    {
      if(curr_ != NULL){
        if(curr_->getRight() != NULL){
          ...
          while(curr_->getParent() != NULL){
            if(curr_->getParent()->getLeft() == curr_){
