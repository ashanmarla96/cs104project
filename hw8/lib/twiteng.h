...
class TwitEng
{
 public:
  ...
  std::vector<trendcount> trendinghelp();
  void trending(std::vector<trendcount>&);
 private:
  ...
  struct trendcount {
      ...
      bool operator==(trendcount& rhs) const {
        return this->hashtag == rhs.hashtag;
      }
    };

  struct heapcomp {
    bool operator()(const trendcount& lhs, const trendcount& rhs) 
    { 
