...
bool TwitEng::parse(char* filename)
{
	...
	while( usr_cnt < num_users && getline(infile, txtLine)){
		...
		{
			myuserptr = new User(myuser, hashpass);
...
vector<trendcount> TwitEng::trendinghelp()
{
	vector<trendcount> currCount;
	...
	vector<Tweet*>::iterator it;
  	for(it = tweets_.begin();it != tweets_.end();++it){
  		...
  		set<string>::iterator ht;
  		for(ht= hashtags.begin(); ht != hashtags.end();++ht){
  			...
  			if(currCount.size() != 0){
  				int counter = 0;
  				vector<trendcount>iterator trendit;
				for(trendit = currCount.begin(); trendit != currCount.end(); ++trendit){
					if(*trendit != temp){
						continue;
					}
					else{
...
void TwitEng::trending(vector<trendcount>& currCount)
{
	heapcomp _c;
  	itemcomp _k;

  	Heap<trendcount, heapcomp, itemcomp> trendingheap(2, _c, _k);

  	for(int i =0; i<currCount.size(); i++){
  		trendingheap.push(currCount[i]);
  	}
  	
  	int temp=0;
  	while((trendingheap.size() !=0)&&(temp <5)){
  		if(temp == 0){
  			cout<<"The trending hash tags are: ";
  		}
  		cout<<(trendingheap.top()).hashtag<<" ";
  		trendingheap.pop();
  		temp++;
  	}
  	cout<<endl;
